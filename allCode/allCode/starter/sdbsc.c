...
int get_student(int fd, int id, student_t *s) {
    lseek(fd, (id - 1) * STUDENT_RECORD_SIZE, SEEK_SET);
    int bytesRead = read(fd, s, STUDENT_RECORD_SIZE);
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    int result = get_student(fd, id, &existing_student);
    ...
    lseek(fd, (id - 1) * STUDENT_RECORD_SIZE, SEEK_SET);
    int bytesWritten = write(fd, &new_student, STUDENT_RECORD_SIZE);
...
int del_student(int fd, int id) {
    ...
    lseek(fd, (id - 1) * STUDENT_RECORD_SIZE, SEEK_SET);
    int bytesWritten = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
